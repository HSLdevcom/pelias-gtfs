...
var altStationNames =  require( './stations.js' );
...
function buildTransportModeString(types) {
  ...
  if (testRouteType(types, val => (val === 900))) {
    names.push('SPEEDTRAM'); // "Raidejokeri"
  }
  ...
  if (testRouteType(types, val => (val === 7 || val === 1400))) {
    names.push('FUNICULAR');
  }
      ...
      if (activeStops.notDefined || activeStops[record.stop_id]) {
        try {
          ...
          if (transportModes) {
            ...
            vehicleType = mapTypes(transportModes);
            if (vehicleType) {
              doc.addCategory(vehicleType);
            }
          }
          if (record.stop_code && record.stop_code !== '') {
            gtfsAdd.code = record.stop_code;
          }
          if (transportModes || gtfsAdd.platform || gtfsAdd.code) {
            doc.setAddendum('GTFS', gtfsAdd);
          }

          var platinfo;
          if (vehicleType === 'train stop') {
            platinfo = track;
          } else {
            platinfo = plat;
          }
          ...
          if (record.platform_code) {
            doc.setNameAlias( 'default', name + platinfo.fi + record.platform_code);
          }
          if(name.indexOf(' (M)') !== -1) {
            doc.setName( 'alternative', name.replace(' (M)', '') );
          } else {
            var key = name.replace(',', '').toLowerCase();
            var altName = altStationNames[key];
            if (altName) {
              doc.setName( 'alternative', altName);
            }
          }
          if(shortName) {
          ...
          if (translations) {
            var names = translations[name] || translations[record.stop_id];
            if (names) {
              for (var lang in names) {
                ...
                if (name2 !== name) {
                  ...
                  if (record.platform_code) {
                    doc.setNameAlias( lang, name2 + platinfo[lang] + record.platform_code);
                  }
                }
              }
            }
